...
if __name__ == '__main__':
    ...
    #pdb.set_trace()
    bangs = np.column_stack((b2d_poses[1][2,0:3], b2d_poses[1][2,4:]))
    b2d_poses[0] = np.column_stack((b2d_poses[0][:, 0:3], b2d_poses[0][:, 4:]))
    ...
    for label, ang_mat in zip(['a','b','path'], [aangs, bangs, pat_angs]):
        ...
        print label, 'mean (deg)', np.degrees(mean), 'std (deg)', np.degrees(std)
    ...
    b_at_distance = np.column_stack((b_at_distance[:, 0:3], b_at_distance[:, 4:]))
    ...
    pb.plot(a_at_distance[0,:].A1, a_at_distance[1,:].A1, 'b*')

    pb.plot(b2d_poses[0][0,:].A1, b2d_poses[0][1,:].A1, 'ro')
    pb.plot(b_at_distance[0,:].A1, b_at_distance[1,:].A1, 'r*')

    pb.xlabel('position (meters)')
    pb.ylabel('position (meters)')
    pb.axis('equal')
