...
def getGoalPose():
    # Get pose from the param server
    while not rospy.has_param("/m3/software_testbed/goal"):
      rospy.sleep(1.0)
      
    goal_position_list = rospy.get_param("/m3/software_testbed/goal")
    
    goal_pose = geometry_msgs.msg.PoseStamped()
    goal_pose.header.frame_id = "torso_lift_link"
    goal_pose.header.stamp = rospy.get_rostime()

    goal_pose.pose.position.x = goal_position_list[0]
    goal_pose.pose.position.y = goal_position_list[1]
    goal_pose.pose.position.z = goal_position_list[2]
    
    goal_pose.pose.orientation.w = 1.0 # The rest of the quaternion (x,y,z) is 0.0
...
if __name__ == '__main__':  
    ...
    goal_pose_pub = rospy.Publisher("/haptic_mpc/goal_pose", geometry_msgs.msg.PoseStamped, latch=True) # Goal pose publisher.
    ...
    while not_done == True:
        ...
        p = subprocess.Popen(['rospack', 'find', 'hrl_tactile_controller'], stdout=subprocess.PIPE)
        controller_dir = p.communicate()[0].rstrip() 
        
        sim_param_file = controller_dir + "/sim_params.yaml"
        mpc_param_file = controller_dir + "/mpc_params.yaml"
        subprocess.Popen(['rosparam', 'load', sim_param_file])
        subprocess.Popen(['rosparam', 'load', mpc_param_file])
        ...
        try:
            while keep_running:
                ...
                if keep_running == False:
                    print "mpc_state :\n", mpc_state
                    print "mpc_error :\n", mpc_error
                if controller.poll() != None or traj_manager.poll() != None or haptic_state.poll() != None or logger.poll() != None or monitor.poll() != None or simulator.poll() != None:# or cvxgen_node.poll() != None:
