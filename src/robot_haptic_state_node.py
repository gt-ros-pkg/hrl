...
class RobotHapticStateServer():
  ...
  def updateEndEffectorJacobian(self):
    self.Je = [self.robot.kinematics.point_jacobian(self.joint_angles, self.end_effector_position)]
  ...
  def updateContactJacobians(self, skin_data):
    ...
    for jt_li, loc_li in it.izip(jt_l, loc_l):
      Jc = self.robot.kinematics.point_jacobian(self.joint_angles, loc_li)
      Jc[:, jt_li+1:] = 0.0
  ...
  def updateEndEffectorPose(self):
    mat = self.robot.kinematics.forward(self.joint_angles)
    self.end_effector_position = mat[:3,3]
    self.end_effector_orient_cart = mat[:3,:3]
    self.end_effector_orient_quat = tr.matrix_to_quaternion(mat[:3,:3])
