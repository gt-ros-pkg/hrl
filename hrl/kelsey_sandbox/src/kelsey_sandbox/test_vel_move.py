...
def main():
    ...
    return
    r_arm_js = create_pr2_arm('r', PR2ArmJointTrajectory)
    ...
    r_arm = create_pr2_arm('r', PR2ArmJTranspose)
    rospy.sleep(0.2)
...
#pos_lead = np.mat([0, 0, 0]).T
    ...
    while not rospy.is_shutdown():
        ...
        xd_ctrl = 0.2 * (xd_des - xd_act_filt) + 0.04 * xd_integ + np.sign(xd_des) * 0.05
        xd_ctrl_final = np.clip(xd_ctrl, -0.30, 0.30)
        msg = Float64MultiArray()
        msg.data = [xd_ctrl, (xd_des - xd_act), xd_integ, xd_ctrl_final, xd_act_filt]
