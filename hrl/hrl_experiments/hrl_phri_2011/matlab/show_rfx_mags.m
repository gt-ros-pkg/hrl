...
for subj = 1:8
    ...
    for treat = 1:4
        ...
        for pt_ind = 1:4
            ...
            pt_above = sum(mags>ptile2) / (sum(mags==mags)*8);
...
for treat = 1:4
    ...
    bar(xbar+xbar_adj, total_hist(treat,:)/8);
    hold on;
    plot([ptile_avg,ptile_avg], [0,10000])
    set(plt, 'Xlim', [0, xbar_max]);
    %set(plt, 'Ylim', [0, 1.3 * max(total_hist(treat,:)/8)]);
    set(plt, 'Ylim', [0, 0.28]);
end
...
plt = subplot(5, 1, 5);
xbar = linspace(0, end_mult, hist_size) * ptile75_avg;
bar(xbar+xbar_adj, train_total_hist/8);
set(plt, 'Xlim', [0, xbar_max]);
%set(plt, 'Ylim', [0, 1.3 * max(train_total_hist/8)]);
set(plt, 'Ylim', [0, 0.28]);
