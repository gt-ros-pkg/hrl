...
def main():
    ...
    arm.set_motion_gains(p_trans=200, d_trans=15, p_rot=3, d_rot=0)
    arm.set_force_gains(p_trans=1, p_rot=0.5)
    ...
    while not rospy.is_shutdown():
        ...
        if np.linalg.norm(err_ep[0:3]) > 0.02 or np.linalg.norm(err_ep[3:]) > np.pi / 16.:
            arm.set_ep(cur_pose, 1)
        r.sleep()
