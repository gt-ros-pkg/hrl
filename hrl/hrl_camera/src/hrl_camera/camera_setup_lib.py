...
############## Connectivity ##################
#            
#def find_connected_camera_ids():
#    '''This function will return a list of camera ids that are currently connected and
#    listed in camera_config'''
#    import camera_config as cc
#    import createUIDIndexDict
#    
#    uidIndexDict = createUIDIndexDict.createDict()
#    init_bus1394()
#
#    connected_cam_ids = []
#    for a_cam_id in cc.camera_parameters:
#        cc_dict = cc.camera_parameters[ a_cam_id ]
#        if cc_dict.has_key('uid'):
#            uid_from_cam_parms = cc_dict['uid']
#            if uidIndexDict.has_key( uid_from_cam_parms ):
#                connected_cam_ids.append( a_cam_id )
#
#    #if there are more cameras on the bus than exist in the dictionary
#    #notify the user
#    if len(connected_cam_ids) != len(uidIndexDict):
#        cams_with_no_settings = []
#
#        for cam_on_bus in uidIndexDict:
#            found_cam_settings = False
#            for a_cam_id in cc.camera_parameters:
#                uid_from_cam_parms = cc.camera_parameters[ a_cam_id ]['uid']
#                if cam_on_bus == uid_from_cam_parms:
#                    found_cam_settings = True
#
#            if found_cam_settings == False:
#                cams_with_no_settings.append(cam_on_bus)
#
#        if len( connected_cam_ids ) == 0:
#            connected_cam_ids = None
#            
#        raise KeyError, 'Bus has device(s) undefined in camera_config! (%s) Connected defined camera ids:     %s'% (repr( cams_with_no_settings ), repr( connected_cam_ids ))
#
#    return connected_cam_ids
#    
#
