...
from hrl_rfh_fall_2011.srv import GetHeadPose, GetHeadPoseRequest
...
class Shaving_manager():
    def __init__(self):
        ...
        rospy.Subscriber('wt_cont_shaving_step', Int8, self.cont_change_pose)
        rospy.Subscriber('wt_shave_location', Int8, self.set_shave_pose)
  ...
  #          self.ft_activity = rospy.Time.now()
    ...
    def cont_change_pose(self, step):
        self.new_pose = True
        ellipse_pose = self.cont_pose_proxy.call(step)
        rospy.loginfo("Received New Pose")
        self.wt_log_out.publish("Received New Pose")
    ...
    def adjust_pose(self, ellipse_pose):
        ...
        if TOOL == 'inline':
            self.goal_pose = self.jtarms.pose_frame_move(torso_pose, -(self.hand_offset + self.inline_tool_offset), arm=0)
            self.approach_pose = self.jtarms.pose_frame_move(goal_pose, -0.15, arm=0)
        elif TOOL == '90':
            self.goal_pose = self.jtarms.pose_frame_move(torso_pose, 0.015)

    def shave(self):
        goal_pose = self.goal_pose
        approach_pose = self.approach_pose
        manager_rate = rospy.Rate(20)
        while not rospy.is_shutdown():
            if goal_pose.header.stamp != self.goal_pose.header.stamp:
                goal_pose = self.goal_pose
                approach_pose = self.approach_pose
...
if __name__=='__main__':
    ...
    if SHAVING_TEST:
        ...
        while not rospy.is_shutdown():
            rospy.spin()
        sm.Shaving_manager.controller_switcher.carefree_switch('l','%s_arm_controller')
