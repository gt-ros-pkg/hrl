...
class Follower:
    ...
    def __init__(self):
        ...
        self.goal_out = rospy.Publisher('/servo_dest', PoseStamped)
    ...
    def set_trans(self):
        if abs(self.ang_diff) < math.pi/6 and self.dist_to_goal > self.dist_thresh: #Facing the right direction and not there yet, so start moving.
    ...
    def slide(self):
       # Determine left/right movement speed for strafing obstacle avoidance 
        ...
        if len(self.left[:][0]) > 0:
           ...
           # print "lefts:\r\n %s" %lefts
            if len(lefts) > 0:
                #left_dists = np.sqrt(np.add(np.square(lefts[:][0]), np.square(lefts[:][1])))
        ...
        if len(self.right[:][0]) > 0:
           ...
           # print "rights:\r\n %s" %rights
            if len(rights) > 0:
                #right_dists = np.sqrt(np.add(np.square(rights[:][0]), np.square(rights[:][1])))
    ...
    def avoid_obstacles(self):
        if len(self.front[0][:]) > 0:
            if min(self.front[0][:]) < self.retreat_thresh: #0.225:  #0.5 (round-up on corner-to-corner radius of robot) - 0.275 (x diff from base laser link to base footprint)
                ...
                front_dists = np.sqrt(np.add(np.square(self.front[0][:]),np.square(self.front[1][:])))
                min_x = self.front[0][np.argmin(front_dists)]
                min_y = self.front[1][np.argmin(front_dists)]
                print "min x/y: %s,%s" %(min_x, min_y)
                self.command.linear.x = -np.sign(min_x)*np.clip(abs(min_x),0.05,0.1) 
                self.command.linear.y = -np.sign(min_y)*np.clip(abs(min_y),0.05,0.1) 
                self.command.angular.z = 0.0
    ...
    def left_clear(self): # Base Laser cannot see obstacles beside the back edge of the robot, which could cause problems, especially just after passing through doorways...
        if len(self.left[0][:])>0:
            ...
            if len(left_obs[0][:])>0:
                ...
                if len(left_obs[:][0])>0:
                    print "Obstacle immediately to the left, cannot move further"
    ...
    def right_clear(self):
        if len(self.right[0][:])>0:
           ...
           if len(right_obs[0][:])>0:
                ...
                if len(right_obs[:][0])>0:
                    print "Obstacle immediately to the right, cannot move further"
                    self.command.linear.y = 0
