...
svm_model *svm_train(const svm_problem *prob, const svm_parameter *param)
{
	...
	else
	{
	    ...
	    //fprintf(stdout, "size of bool %d %d %d", sizeof(bool), sizeof(unsigned int), sizeof(unsigned char));	
        model->SVidx = (unsigned char*) nonzero;
		free(label);
		free(probA);
		free(probB);
		free(count);
...
double svm_predict_values(const svm_model *model, const svm_node *x, double* dec_values)
{
	...
	else
	{
		...
		int p=0;
        //fprintf(stdout, "nr_class %d", nr_class);
		for(i=0;i<nr_class;i++)
			for(int j=i+1;j<nr_class;j++)
			{
				double sum = 0;
				...
				sum -= model->rho[p];
				dec_values[p] = sum;
                //fprintf(stdout, "dec_values %d vs %d result %d %.3f\n", i, j, p, dec_values[p]);
