...
class SimpleArmManager(object):
    ...
    def set_joint_angles(self, arm, q, duration=0.15):
        if arm != 1:
            arm = 0
        self.jtg = JointTrajectoryGoal()
        self.jtg.trajectory.joint_names = self.joint_names_list[arm]
        jtp = JointTrajectoryPoint()
        jtp.positions = q
        jtp.velocities = [0 for i in range(len(q))]
        jtp.accelerations = [0 for i in range(len(q))]
        jtp.time_from_start = rospy.Duration(duration)
        self.jtg.trajectory.points.append(jtp)
    ...
    def move_arm_trajectory(self, arm, pos_arr, dur=1., rot_arr=None, dur_arr=None):
        ...
        for i in range(len(pos_arr)):
            ...
            if not (nextq is None):
                curq = nextq
        print "IK Accuracy: ", trys-fails, "/", trys, "(", (trys-fails)/float(trys), ")"
