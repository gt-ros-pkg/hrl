...
namespace hrl_object_fetching {
    ...
    void TabletopDetector::onInit() {
        ...
        nh_priv.param<double>("max_table_height", minz, 1.0);
        ...
        nh_priv.param<double>("xgran", xgran, 0.1);
        nh_priv.param<double>("ygran", ygran, 0.1);
        ...
        if(run_service)
            ...
            pc_sub = nh.subscribe("/kinect_head/rgb/points", 10, &TabletopDetector::pcCallback, this);
    ...
    void TabletopDetector::pcCallback(sensor_msgs::PointCloud2::ConstPtr pc_msg) {
        ...
        /////////////////// Project top heights onto x-y plane ////////////////
        ...
        cv::Mat cur_count = (cur_height_img > 0) / 255;
        cur_count.convertTo(cur_count, CV_32F);
        height_img_count += cur_count;
        ...
        height_img_avg.convertTo(height_img_avg, CV_8U);
        printf("yo %d",yo++);

        ////////////////////// Height image histogram /////////////////////////
        ...
        for(uint32_t x=0;x<imgx;x++)
            for(uint32_t y=0;y<imgy;y++) {
                ...
                if(height_img_avg.at<uint8_t>(x,y) != 0) {
                    height_hist.at<float>(std::floor(height_img_avg.at<uint8_t>(x,y)), 0)++;
                }
            }
        printf("yo %d",yo++);
        ...
        //////////////// Mophological operations to clean image //////////
        ...
        /////////////////  PCA on existing points ///////////////////////
        double sumx = 0, sumy = 0, surf_area = 0;
        for(uint32_t y=0;y<imgy;y++) {
        ...
        double centerx = sumx/surf_area, centery = sumy/surf_area;
        ...
        ///////////////// Find objects above table and avg location /////////////
        ...
        for(uint32_t i=0;i<posesx.size();i++) {
            ...
            for(int32_t j=((int)retposesx.size())-1;j>=0;j--) {
                if(fabs(posesx[i] - retposesx[j]) < xgran && 
                   fabs(posesy[i] - retposesy[j]) < ygran) {
                    same_found = true;
                } 
            }
            if(!same_found) {
