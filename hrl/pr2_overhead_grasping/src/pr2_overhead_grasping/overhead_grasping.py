...
def write_model_index_file(prefix = GRASP_IND_PREFIX, index = GRASP_INDEX_FILE):
    grasps = []
...
def generate_mean_models():
    grasps = []
    data_index = load_pickle(GRASP_DATA_INDEX_FILE)
    num_gen = 0
    for grasp_i in data_index:
        print "Number of datasets smoothed:", num_gen
        num_gen += 1
        grasp = load_pickle(grasp_i[1])
        grasp = [grasp[0], grasp[1], grasp[2], generate_mean_grasp(grasp[2]), grasp[4]]
        save_grasp(grasp, GRASP_DATA_PREFIX)

def generate_space_models(ki):
    if False:
        generate_mean_models()
    data_index = load_pickle(GRASP_DATA_INDEX_FILE)
    ...
    for i in range(NUM_X - 1):
            ...
            for k in range(NUM_ROT):
                ...
                if True or num_gen >= 230:
                    ...
                    if len(close_models) > 8:
                        print len(close_models), "problem"
                        print st
                        return
        ...
        st[0] += xs
        st[1] = RECT[1][1] + ys/2.
    write_index_file(prefix = GRASP_IND_PREFIX, index = GRASP_INDEX_FILE)
...
def display_grasp_data_new(grasps, percept="accelerometer", indicies=range(3), 
...
def resample_split_model_data(apm, models=None, sampling_rate=RESAMPLE_RATE):
    if models is None:
...
def resample_model_data(apm, sampling_rate=RESAMPLE_RATE):
    xyr_index = load_pickle(GRASP_INDEX_FILE)
    for xyr in xyr_index:
        grasp = load_pickle(xyr[1])
        new_model = apm.resample_and_thin_data(grasp[3], sampling_rate)
        save_pickle([grasp[0], None, None, new_model, grasp[4]], xyr[1])
...
def main(args):
    ...
    #return 0
    #grasp = load_pickle("model_indexed//grasp_model0047--022-0287.pickle")
    #display_grasp_data_new([grasp], "accelerometer")
    #return 0
    
    # write_model_index_file(prefix = GRASP_IND_PREFIX, index = GRASP_INDEX_FILE)
    # return 0

    # gi = load_pickle(GRASP_INDEX_FILE)
    # print gi
    # return 0
